...
doesn’t want to see every location where \cmd{if} is used—or \cmd{the} or \cmd{let} or
\cmd{begin} or any of numerous other control sequences.

As its name implies, the \cmd{DoNotIndex} command gives Doc a list of control
sequences that should not be indexed. \cmd{DoNotIndex} can be used any
...
package names to be typeset with \cmd{textsf} and for \cmd{thanks} to be used to
