...
\sidenote{Most of this discussion is based on an article by Stephan v. Bechtolsheim see \url{http://www.tug.org/TUGboat/Articles/tb10-2/tb24bechtolsheim.pdf}}
...
such as digits. \sidenote{Normal macro definitions cannot contain any digits, but just alphanumeric characters}
...
\begin{multicols}{2}
It is important to stress that|\csname| does not define anything: you need to use the TeX primitive \doccmd{def} to create a definition. This also requires the \doccmd{expandafter} primitive.
\end{multicols}
...
If you check the character code |\the\catcode`*|, this will give you 12, which has a category code of  other, so in order to use it, and note we don't normally use it like this we need to change its category code. 
